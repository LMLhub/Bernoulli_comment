...
\usepackage{xspace,graphicx,graphics,xcolor}
...
%Markowitz [29] was the first to propose that utility be defined on gains and losses rather than on final asset positions, an assumption which has been implicitly accepted in most experimental measurements of utility (see, e.g., [7, 32]). [KT1979, p.276]
...
\appendix{\bf Appendix A: Ex falso quodlibet}
...
\appendix{\bf Appendix B: linear functions imply trivial expectation value}
\begin{proof}
\bea
...
\appendix{\bf Appendix C: Trivial expectation value implies linear function}
\begin{proof}
...
\appendix{\bf Appendix D: insensitivity to $x_0$ implies linear utility}
\begin{prop}
